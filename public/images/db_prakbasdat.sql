...
--
-- Host: localhost:3306
-- Generation Time: Jun 15, 2025 at 10:04 AM
...
,`lokasi_update` varchar(100)
,`status_paket` varchar(50)
,`status_saat_ini` varchar(50)
,`tanggal_update` timestamp
,`total_paket` bigint
,`total_paket_pengantaran` bigint
,`total_paket_terkirim` bigint
,`total_paket_transit` bigint
);

-- --------------------------------------------------------

--
-- Stand-in structure for view `data_paket`
-- (See below for the actual view)
--
CREATE TABLE `data_paket` (
`Berat (KG)` decimal(10,2)
,`Jenis Layanan` varchar(50)
,`Kota Asal` varchar(50)
,`Kota Tujuan` varchar(50)
,`Ongkos Kirim` decimal(10,2)
);
...
(2, 'CREATE', '2025-06-15 00:11:41', 'Data kota baru ditambahkan dengan ID: 2 dan Nama: Jakarta');
...
(2, 'UPDATE', '2025-06-15 00:12:51', 'Paket dengan ID: PKT001 diperbarui. Status lama: Dalam Perjalanan, Status baru: Dalam Perjalanan'),
(3, 'CREATE', '2025-06-15 00:29:40', 'Paket baru ditambahkan dengan ID: PKT002. Pengirim: 1, Penerima: 1');
...
(2, 'CREATE', '2025-06-15 00:49:55', 'Data pelacakan baru ditambahkan: 3'),
(3, 'UPDATE', '2025-06-15 00:51:02', 'Data pelacakan diperbarui: 3'),
(4, 'UPDATE', '2025-06-15 03:11:02', 'Data pelacakan diperbarui: 3'),
(5, 'UPDATE', '2025-06-15 03:12:16', 'Data pelacakan diperbarui: 3'),
(6, 'UPDATE', '2025-06-15 03:12:46', 'Data pelacakan diperbarui: 3'),
(7, 'UPDATE', '2025-06-15 03:13:10', 'Data pelacakan diperbarui: 3'),
(8, 'UPDATE', '2025-06-15 03:14:42', 'Data pelacakan diperbarui: 3'),
(9, 'UPDATE', '2025-06-15 03:16:22', 'Data pelacakan diperbarui: 3'),
(10, 'UPDATE', '2025-06-15 03:22:15', 'Data pelacakan diperbarui: 1'),
(11, 'UPDATE', '2025-06-15 03:36:44', 'Data pelacakan diperbarui: 1'),
(12, 'UPDATE', '2025-06-15 03:36:55', 'Data pelacakan diperbarui: 1'),
(13, 'UPDATE', '2025-06-15 03:40:28', 'Data pelacakan diperbarui: 3'),
(14, 'UPDATE', '2025-06-15 03:45:43', 'Data pelacakan diperbarui: 1');
...
CREATE TABLE `paket` (
  ...
  `id_layanan` int NOT NULL,
  `id_kota_asal` int DEFAULT NULL,
  `id_kota_tujuan` int DEFAULT NULL,
...
INSERT INTO `paket` (`id_paket`, `id_pengirim`, `id_penerima`, `id_layanan`, `id_kota_asal`, `id_kota_tujuan`, `berat_paket`, `tanggal_pengiriman`, `ongkos_kirim`, `status_paket`, `estimasi_tiba`, `catatan_paket`) VALUES
('PKT001', 1, 1, 1, 1, 2, '5.00', '2025-06-13 15:45:27', '25000.00', 'Dalam Perjalanan', '2025-06-15', 'Jangan dibanting'),
('PKT002', 1, 1, 1, 2, 1, '10.00', '2025-06-15 00:29:40', '25000.00', 'Transit', '2025-06-18', 'Dikirim dalam kondisi baik');
...
(1, 'PKT001', '2025-06-13 15:45:27', 'Gudang Bandung', 'Dalam Pengantaran', 1, 'Paket sedang diproses'),
(3, 'PKT002', '2025-06-15 00:49:55', 'Gudang Jakarta', 'Transit', 1, 'Paket baru saja dikirim dari gudang');
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `dashboard`  AS SELECT `p`.`id_paket` AS `id_paket`, `p`.`status_paket` AS `status_paket`, `kota_asal`.`nama_kota` AS `kota_asal`, `kota_tujuan`.`nama_kota` AS `kota_tujuan`, `t`.`tanggal_update` AS `tanggal_update`, `t`.`lokasi_update` AS `lokasi_update`, `t`.`status_saat_ini` AS `status_saat_ini`, (select count(0) from `paket`) AS `total_paket`, (select count(0) from `pelacakan` where (`pelacakan`.`status_saat_ini` = 'Terkirim')) AS `total_paket_terkirim`, (select count(0) from `pelacakan` where (`pelacakan`.`status_saat_ini` = 'Transit')) AS `total_paket_transit`, (select count(0) from `pelacakan` where (`pelacakan`.`status_saat_ini` = 'Dalam Pengantaran')) AS `total_paket_pengantaran` FROM (((`paket` `p` left join `kota` `kota_asal` on((`p`.`id_kota_asal` = `kota_asal`.`id_kota`))) left join `kota` `kota_tujuan` on((`p`.`id_kota_tujuan` = `kota_tujuan`.`id_kota`))) left join `pelacakan` `t` on((`p`.`id_paket` = `t`.`id_paket`))) ORDER BY `t`.`tanggal_update` AS `DESCdesc` ASC  ;

-- --------------------------------------------------------

--
-- Structure for view `data_paket`
--
DROP TABLE IF EXISTS `data_paket`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `data_paket`  AS SELECT `p`.`berat_paket` AS `Berat (KG)`, `l`.`nama_layanan` AS `Jenis Layanan`, `s`.`kota_pengirim` AS `Kota Asal`, `r`.`kota_penerima` AS `Kota Tujuan`, `p`.`ongkos_kirim` AS `Ongkos Kirim` FROM (((`paket` `p` join `layanan` `l` on((`p`.`id_layanan` = `l`.`id_layanan`))) join `pengirim` `s` on((`p`.`id_pengirim` = `s`.`id_pengirim`))) join `penerima` `r` on((`p`.`id_penerima` = `r`.`id_penerima`)))  ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `laporan`  AS SELECT `p`.`id_paket` AS `id_paket`, `pg`.`nama_pengirim` AS `pengirim`, `pn`.`nama_penerima` AS `penerima`, `ka`.`nama_kota` AS `kota_asal`, `kt`.`nama_kota` AS `kota_tujuan`, `p`.`berat_paket` AS `berat_paket`, `p`.`ongkos_kirim` AS `ongkos_kirim`, `pl`.`status_saat_ini` AS `status`, `pl`.`tanggal_update` AS `tanggal` FROM (((((`paket` `p` join `pengirim` `pg` on((`p`.`id_pengirim` = `pg`.`id_pengirim`))) join `penerima` `pn` on((`p`.`id_penerima` = `pn`.`id_penerima`))) join `kota` `ka` on((`p`.`id_kota_asal` = `ka`.`id_kota`))) join `kota` `kt` on((`p`.`id_kota_tujuan` = `kt`.`id_kota`))) join `pelacakan` `pl` on((`p`.`id_paket` = `pl`.`id_paket`))) WHERE (`pl`.`tanggal_update` = (select max(`pl2`.`tanggal_update`) from `pelacakan` `pl2` where (`pl2`.`id_paket` = `p`.`id_paket`)))  ;
...
--
ALTER TABLE `kota`
  MODIFY `id_kota` int NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=3;
...
--
ALTER TABLE `pelacakan`
  MODIFY `id_pelacakan` int NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=4;
...
--
ALTER TABLE `paket`
  ADD CONSTRAINT `fk_kota_asal` FOREIGN KEY (`id_kota_asal`) REFERENCES `kota` (`id_kota`),
  ADD CONSTRAINT `fk_kota_tujuan` FOREIGN KEY (`id_kota_tujuan`) REFERENCES `kota` (`id_kota`),
